...
export class RateResponseDto {
    @ApiProperty({ description: 'ID of the skill associated with the response', example: 101 })
    @IsInt()
    @Min(1)
    skillId: number;

    @ApiProperty({ description: 'Difficulty level of the question', enum: ['easy', 'medium', 'hard'], example: 'medium' })
    ...
    @ApiProperty({ description: 'Rating given to the response', minimum: 1, maximum: 5, example: 4 })
    @IsInt()
    @Min(1)
    rating: number;
